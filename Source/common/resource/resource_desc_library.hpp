...
public:
    ...
    template<typename T>
    ResourceDescLibrary& add(const char* ext, int flags, create_resource_doc_fn_t fn, create_preview_fn_t preview_fn = nullptr) {
    ...
    template<typename T>
    ResourceDescLibrary& add(const std::vector<std::string>& extensions, int flags, create_resource_doc_fn_t fn, create_preview_fn_t preview_fn = nullptr) {
        for(auto& ext : extensions) {
            add<T>(ext.c_str(), flags, fn, preview_fn);
        }
        return *this;
    }
