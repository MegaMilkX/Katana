...
public:
    bool is_valid_triangulation = false;
    std::vector<double> const& coords;
    ...
    bool is_valid() const { return is_valid_triangulation; }
...
private:
    ...
    std::size_t add_triangle(
        ...
        std::size_t b,
        std::size_t c);
    bool link(std::size_t a, std::size_t b);
};
...
inline Delaunator::Delaunator(std::vector<double> const& in_coords)
    ...
    auto t = add_triangle(i0, i1, i2, INVALID_INDEX, INVALID_INDEX, INVALID_INDEX);
    if(t == INVALID_INDEX) {
        return;
    }
...
inline std::size_t Delaunator::add_triangle(
    ...
    if(!link(t, a) || !link(t + 1, b) || !link(t + 2, c)) {
        return INVALID_INDEX;
    }
    return t;
}

inline bool Delaunator::link(const std::size_t a, const std::size_t b) {
