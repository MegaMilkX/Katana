...
static inline bool isPreprocessor(int ch) {
    return ch == '#';
}

static inline bool isStringLiteral(int ch) {
    return ch == '"';
}
...
void lexer::read(token &out) {
    ...
    } else if(out.m_type == kType_preprocessor) {
        free(out.asPreprocessorDirective);
    } else if (out.m_type == kType_string_literal) {
        free(out.asStringLiteral);
    }
    ...
    } else if (isPreprocessor(at())) {
        out.m_type = kType_preprocessor;
        std::vector<char> str;
        str.push_back(at());
        m_location.advanceColumn();
        while (position() != m_length && isChar(at())) {
            str.push_back(at());
            m_location.advanceColumn();
        }
        str.push_back('\0');

        out.asPreprocessorDirective = (char *)malloc(str.size());
        if (!out.asPreprocessorDirective) {
            m_error = "Out of memory";
            return;
        }
        memcpy(out.asPreprocessorDirective, &str[0], str.size());
    } else if (isStringLiteral(at())) {
        out.m_type = kType_string_literal;
        std::vector<char> str;
        str.push_back(at());
        m_location.advanceColumn();
        while (position() != m_length && at() != '"') {
            str.push_back(at());
            m_location.advanceColumn();
        }
        if (at() == '"') {
            ...
            m_error = "Reached end of file before end of string literal";
            return;
        }
        str.push_back('\0');
        out.asStringLiteral = (char *)malloc(str.size());
        if (!out.asStringLiteral) {
            m_error = "Out of memory";
            return;
        }
        memcpy(out.asStringLiteral, &str[0], str.size());
    } else {
        switch (at()) {
        // Non operators
        case '\n':
    ...
    out.length = m_location.position - out.pos;
}
