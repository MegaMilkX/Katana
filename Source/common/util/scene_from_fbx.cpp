...
const aiScene* loadAssimpScene(const void* buf, size_t sz, const std::string& filename_hint, std::vector<aiMesh*>& meshes_with_fallback_uv) {
...
void freeAssimpScene(const aiScene* ai_scene, std::vector<aiMesh*>& meshes_with_fallback_uv) {
    // Cleanup
...
bool sceneFromFbx(const std::vector<char>& buffer, GameScene* scn, const std::string& filename = "") {
    ...
    const aiScene* ai_scene = loadAssimpScene(buffer.data(), buffer.size(), filename, meshes_with_fallback_uv);
    ...
    createGraphFromAssimpNode(ai_rootNode, scn->getRoot());
    scn->getRoot()->setName(root_name);
    scn->getRoot()->getTransform()->setScale((float)scaleFactor);
    resourcesFromAssimpScene(ai_scene, asset_params, fname, root_name);
    finalizeObjectsFromAssimpNode(ai_scene, ai_rootNode, scn->getRoot(), fname, root_name);

    if(ai_scene->mNumAnimations > 0) {
        auto anim_stack = scn->getRoot()->get<AnimationStack>();
    ...
    scn->getRoot()->refreshAabb();

    freeAssimpScene(ai_scene, meshes_with_fallback_uv);
    return true;
}
...
bool sceneFromFbx(const std::string& filename, GameScene* scn) {
    ...
    return sceneFromFbx(buffer, scn, filename);
}
